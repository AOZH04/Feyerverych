(() => {
    ...
    function menuInit() {
        ...
        if (document.querySelector(".menubottom_burger")) document.addEventListener("click", (function(e) {
            if (bodyLockStatus && e.target.closest(".menubottom_burger")) {
                bodyLockToggle();
                document.documentElement.classList.toggle("menu-open");
            }
        }));
    ...
    da.init();
    let btnsCatalog = document.querySelectorAll(".btncatalog");
    if (btnsCatalog) for (let index = 0; index < btnsCatalog.length; index++) {
        const btnCatalog = btnsCatalog[index];
        btnCatalog.addEventListener("click", (function(e) {
            e.preventDefault();
            btnCatalog.classList.toggle("catalogopen");
            btnCatalog.parentNode.classList.toggle("catalogopen");
            let mains = document.querySelectorAll(".main");
            if (mains) for (let index = 0; index < mains.length; index++) {
                const main = mains[index];
                main.addEventListener("click", (function() {
                    btnCatalog.classList.remove("catalogopen");
                    btnCatalog.parentNode.classList.remove("catalogopen");
                }));
            }
        }));
    }
    let iconBurgers = document.querySelectorAll(".menubottom_burger");
    if (iconBurgers) for (let index = 0; index < iconBurgers.length; index++) {
        iconBurgers[index];
    }
    window["FLS"] = false;
    menuInit();
